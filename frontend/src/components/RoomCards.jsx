...
export default function RoomCards({ onBookRoom }) {
  ...
  return (
    <section id="rooms" className="py-24 px-4 sm:px-6 bg-[#EFECE6]/50">
          ...
          {ROOM_DATA.map((room, index) => (
            ...
            >
              <div>
                ...
                <RevealTile className="relative h-72 overflow-hidden">
                  <img
                    src={room.image}
                    alt={room.name}
                    className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 ease-out"
                  />
                  <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-md px-3.5 py-1 rounded-full text-[11px] uppercase font-mono tracking-widest text-[#5E6B4F] font-semibold shadow-sm">
                    {room.category}
                  </div>
                  <div className="absolute bottom-4 right-4 bg-[#211F1A]/90 text-white backdrop-blur-md px-4 py-1.5 rounded-full text-xs font-medium tracking-wider shadow-md group-hover:bg-[#9C7A50] transition-colors">
                    from <span className="font-mono text-[#9C7A50] group-hover:text-white font-bold text-sm">€{room.price}</span> / night
                  </div>
                </RevealTile>
                ...
                <div className="p-7 space-y-4">
                  ...
                  <p className="text-xs sm:text-sm text-[#6B6A62] font-light leading-relaxed min-h-[40px]">
                    {room.description}
                  </p>

                  {/* Specs Pill Row */}
                  <div className="flex items-center gap-6 text-xs text-[#6B6A62] border-y border-[#E4DFD2] py-3">
                    <div className="flex items-center gap-1.5">
                      <Maximize2 className="w-4 h-4 text-[#5E6B4F]" />
                      <span>{room.size} m²</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Users className="w-4 h-4 text-[#5E6B4F]" />
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    {room.amenities.slice(0, 3).map((item, idx) => (
                      <span key={idx} className="inline-flex items-center gap-1 bg-[#F6F3EC] text-[11px] text-[#211F1A]/80 px-2.5 py-1 rounded-full border border-[#E4DFD2] group-hover:border-[#5E6B4F]/40 transition-colors">
              ...
              <div className="p-7 pt-0 flex items-center gap-3">
                <button
                  ...
                  data-cursor="Enquire"
                  className="flex-1 border border-[#E4DFD2] hover:border-[#9C7A50] text-[#211F1A] hover:text-[#9C7A50] text-xs font-semibold uppercase tracking-wider py-3 rounded-xl transition-all duration-300 transform active:scale-95"
                >
                  Enquire
                </button>
                <button
                  ...
                  data-cursor="Book"
                  className="flex-1 bg-[#9C7A50] hover:bg-[#85653E] text-white text-xs font-semibold uppercase tracking-wider py-3 rounded-xl transition-all duration-300 shadow-sm hover:shadow-md flex items-center justify-center gap-1.5 group/btn transform active:scale-95"
                >
                  <span>Book Now</span>
          ...
          <div className="bg-white rounded-3xl border border-[#E4DFD2] max-w-2xl w-full overflow-hidden shadow-2xl relative animate-scale-up">
            ...
            <div className="h-64 sm:h-72 relative">
              ...
              <div className="absolute bottom-6 left-6 right-6 text-white space-y-1">
                <span className="text-xs uppercase tracking-widest text-[#9C7A50] font-semibold bg-white/90 px-3 py-1 rounded-full">
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="space-y-2">
                <h4 className="text-xs uppercase tracking-widest font-semibold text-[#9C7A50]">
                  Suite Amenities & Inclusions
                </h4>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {activeModalRoom.amenities.map((am, idx) => (
                    <div key={idx} className="flex items-center gap-2 text-xs text-[#211F1A]/80 bg-[#F6F3EC] p-2 rounded-lg border border-[#E4DFD2]">
                      <Check className="w-3.5 h-3.5 text-[#5E6B4F]" />
                      <span>{am}</span>
                    </div>
                  ))}
                </div>
              ...
              <div className="pt-4 border-t border-[#E4DFD2] flex items-center justify-between">
                <div>
                  <span className="text-xs text-[#6B6A62]">Starting rate</span>
                  <p className="font-serif-luxury text-2xl text-[#211F1A]">
                    €{activeModalRoom.price} <span className="text-xs font-sans text-[#6B6A62]">/ night</span>
                  </p>
                </div>
                ...
                <button
                  ...
                  className="bg-[#9C7A50] hover:bg-[#85653E] text-white text-xs font-semibold uppercase tracking-wider px-8 py-3.5 rounded-xl transition-all duration-300 shadow-md hover:shadow-lg flex items-center gap-2 transform active:scale-95"
                >
