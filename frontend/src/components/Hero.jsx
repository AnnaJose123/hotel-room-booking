...
export default function Hero({ onBookClick }) {
  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center justify-center pt-28 pb-16 overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto px-6 text-center text-white space-y-6 animate-fade-in mt-12">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-xs tracking-[0.25em] uppercase font-light text-[#F7F5F0]">
          <span className="w-1.5 h-1.5 rounded-full bg-[#7A8A6F] animate-pulse" />
        ...
        <div className="pt-4 flex flex-col sm:flex-row items-center justify-center gap-4">
          <button
            ...
            className="w-full sm:w-auto bg-[#7A8A6F] hover:bg-[#68775D] text-white text-xs font-medium uppercase tracking-[0.2em] px-8 py-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-0.5"
          >
            Book Your Stay
          </button>
          ...
          <button
            ...
            className="w-full sm:w-auto border border-white/40 hover:border-white text-white hover:bg-white/10 text-xs font-medium uppercase tracking-[0.2em] px-8 py-4 rounded-full transition-all duration-300"
          >
            Explore Suites
          </button>
        </div>
      ...
      <div className="absolute bottom-8 left-6 right-6 z-20 max-w-[1400px] mx-auto flex justify-between items-center text-white/70">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={prevSlide}
            className="p-2 rounded-full border border-white/20 bg-black/20 backdrop-blur-sm hover:bg-white/20 text-white transition-colors"
